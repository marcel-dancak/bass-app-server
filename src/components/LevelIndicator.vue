<template>
  ...
  <svg class="level-indicator" viewBox="0 0 100 35">
    ...
    <path d="M0 0.5 L95.5 0.5 L99.5 8 L95.5 15.5 L0 15.5 L3 8 Z" stroke="#ddd" fill="#eee" />
...
<script>
  ...
  export default {
    ...
    computed: {
      x() {
        if (this.value) {
          const x = 100*Constants.Difficulties[this.value].numeric/5 - 4
          // small correction of 'hard' level
          return (this.value === 6)? x - 3 : x
        }
        return 0
      },
      label() {
      ...
      activePath() {
        ...
        return `M0.5 1 L${x} 1 L${x+4} 8 L${x} 15 L0.5 15 L3 8 Z`
      }
    }
  }
</script>
